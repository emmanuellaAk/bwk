...
interface DemoProduct { name: string; qty: string; amt: number }
interface DemoOrder   { date: string; total: number; items: number; status: 'paid' | 'outstanding' }
interface DemoSupplier {
  name: string; location: string; contact: string; spent: number
  orders: number; lastOrder: string; products: DemoProduct[]; history: DemoOrder[]
}

const DEMO_SUPPLIERS: DemoSupplier[] = [
  {
    name: 'Royal Hair Supplies', location: 'Accra, GH', contact: '+233 20 900 1122',
    spent: 3840, orders: 11, lastOrder: 'Jul 17',
    products: [
  ...
  {
    name: 'Akosombo Braids Co', location: 'Kumasi, GH', contact: '+233 24 445 6789',
    spent: 1260, orders: 4, lastOrder: 'Jun 30',
    products: [
  ...
  {
    name: 'Tema Extensions Hub', location: 'Tema, GH', contact: '+233 26 667 3344',
    spent: 580, orders: 2, lastOrder: 'Mar 12',
    products: [
...
function ApiDetailPanel({ supplierId, onOrder }: { supplierId: string; onOrder: (name: string) => void }) {
  ...
  if (isLoading) {
    return (
      <div className="flex-1 overflow-y-auto bos-scroll p-6 hidden lg:flex flex-col gap-4">
        {[1, 2].map(i => <div key={i} className="h-[160px] bg-surface-2 rounded-[18px] animate-pulse" />)}
      </div>
    )
  }
  if (!data) return null
...
function ApiDetail({ sup, onOrder }: { sup: SupplierDetail; onOrder: (name: string) => void }) {
  ...
  // Group purchases into "orders" by date (same occurred_at = same order)
  const orderMap = new Map<string, { total: number; items: number; date: string }>()
  for (const p of sup.purchases) {
    const key = p.occurred_at.slice(0, 10)
    const ex  = orderMap.get(key) ?? { total: 0, items: 0, date: new Date(p.occurred_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) }
    orderMap.set(key, { ...ex, total: ex.total + p.total, items: ex.items + 1 })
  }
  const orders = [...orderMap.values()].sort((a, b) => b.date.localeCompare(a.date))

  // Aggregate products bought
  const productMap = new Map<string, { color: string; length: string; qty: number; spend: number }>()
  for (const p of sup.purchases) {
    if (!p.color) continue
    const key = `${p.color} ${p.length}`
    const ex  = productMap.get(key) ?? { color: p.color ?? '', length: p.length ?? '', qty: 0, spend: 0 }
    productMap.set(key, { ...ex, qty: ex.qty + p.quantity, spend: ex.spend + p.total })
  }
    ...
    <div className="flex-1 overflow-y-auto bos-scroll p-6 hidden lg:block">
      ...
      <div className="rounded-[20px] p-[20px] mb-5 text-white" style={{ background: 'linear-gradient(150deg,#6E1B3A,#8A2348)' }}>
        <div className="flex items-center gap-[13px] mb-5">
          ...
          <div>
            <div className="font-serif font-semibold text-[20px] leading-tight">{sup.name}</div>
            <div className="text-[12px] opacity-85 mt-[3px]">{sup.location ?? '—'} · {sup.contact_phone ?? '—'}</div>
          </div>
        </div>
        <div className="flex gap-[28px]">
          <div>
            <div className="text-[11px] opacity-75 font-semibold">Total spent</div>
            <div className="font-serif font-bold text-[22px] mt-[3px]">{cedi(sup.total_spent)}</div>
          </div>
          <div>
            <div className="text-[11px] opacity-75 font-semibold">Orders</div>
            <div className="font-serif font-bold text-[22px] mt-[3px]">{sup.order_count}</div>
          </div>
          <div>
            <div className="text-[11px] opacity-75 font-semibold">Last order</div>
            <div className="font-serif font-bold text-[18px] mt-[5px]">{lastOrderDate}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white border border-line rounded-[18px] p-5">
          ...
          {products.length === 0 ? (
            <div className="text-center py-4 text-muted text-[12.5px]">No purchases yet</div>
          ) : (
            <div className="flex flex-col gap-[9px]">
              {products.map(p => (
                <div key={`${p.color}-${p.length}`} className="flex items-center justify-between bg-surface-2 rounded-[13px] px-[13px] py-[11px]">
                  <div>
                    <div className="font-bold text-[13.5px] text-ink">{p.color} {p.length}</div>
                    <div className="text-[11.5px] text-muted mt-[2px]">{p.qty} packs · {cedi(p.spend / p.qty)}/pack</div>
                  </div>
                  <div className="font-bold text-[13.5px] font-serif text-ink">{cedi(p.spend)}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white border border-line rounded-[18px] p-5">
          ...
          {orders.length === 0 ? (
            <div className="text-center py-4 text-muted text-[12.5px]">No orders yet</div>
          ) : (
            <div className="flex flex-col gap-[9px]">
    ...
    <div className="flex-1 overflow-y-auto bos-scroll p-6 hidden lg:block">
      <div className="rounded-[20px] p-[20px] mb-5 text-white" style={{ background: 'linear-gradient(150deg,#6E1B3A,#8A2348)' }}>
        <div className="flex items-center gap-[13px] mb-5">
          ...
          <div>
            <div className="font-serif font-semibold text-[20px] leading-tight">{sup.name}</div>
            <div className="text-[12px] opacity-85 mt-[3px]">{sup.location} · {sup.contact}</div>
          </div>
        </div>
        <div className="flex gap-[28px]">
          <div>
            <div className="text-[11px] opacity-75 font-semibold">Total spent</div>
            <div className="font-serif font-bold text-[22px] mt-[3px]">{cedi(sup.spent)}</div>
          </div>
          <div>
            <div className="text-[11px] opacity-75 font-semibold">Orders</div>
            <div className="font-serif font-bold text-[22px] mt-[3px]">{sup.orders}</div>
          </div>
          <div>
            <div className="text-[11px] opacity-75 font-semibold">Last order</div>
            <div className="font-serif font-bold text-[18px] mt-[5px]">{sup.lastOrder}</div>
          </div>
        </div>
      </div>
...
export function SuppliersPage() {
  ...
  const demoSup    = DEMO_SUPPLIERS[selectedIdx]
  const apiList    = apiSuppliers ?? []
  const listCount  = isDemo ? DEMO_SUPPLIERS.length : apiList.length
      ...
      <div className="flex flex-col border-r border-line bg-surface overflow-y-auto bos-scroll flex-none w-full lg:w-[320px]">
        <div className="p-5 pb-3">
          ...
          <div className="text-[12px] text-muted font-semibold mb-4">
            {listCount} partner{listCount !== 1 ? 's' : ''}{isDemo ? ' · Demo data' : ''}
          </div>
        </div>

        <div className="flex flex-col gap-[9px] px-5 pb-5">
          {/* Loading skeletons */}
          {hasToken && isLoading && [1, 2, 3].map(i => (
            <div key={i} className="h-[74px] bg-surface-2 rounded-[16px] animate-pulse" />
          ))}

          {/* API list */}
          {!isDemo && !isLoading && apiList.length === 0 && (
            <div className="text-center py-8 text-muted text-[13px]">No suppliers added yet</div>
          )}
          {!isDemo && apiList.map((sup, i) => (
            <button
              key={sup.id}
            ...
            >
              <div className={cn('w-[44px] h-[44px] rounded-[14px] flex items-center justify-center flex-none', selectedIdx === i ? 'bg-plum' : 'bg-surface-2')}>
                <TruckIcon color={selectedIdx === i ? 'white' : '#6E1B3A'} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-[14px] text-ink truncate">{sup.name}</div>
                <div className="text-[11.5px] text-muted mt-[2px]">{sup.location ?? '—'} · {sup.order_count} order{sup.order_count !== 1 ? 's' : ''}</div>
              </div>
              <div className="text-right flex-none">
                <div className="font-bold text-[13.5px] font-serif text-ink">{cedi(sup.total_spent)}</div>
          ...
          {/* Demo list */}
          {isDemo && DEMO_SUPPLIERS.map((sup, i) => (
            <button
              key={sup.name}
              onClick={() => setSelectedIdx(i)}
              className={cn(
