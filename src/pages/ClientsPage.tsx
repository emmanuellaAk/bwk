...
import { useClients, useUpdateClient, useClientAppointments, type ClientRecord } from '@/lib/api/hooks/useClients'
...
function ClientDetail({
  ...
  const { data: appointments, isLoading: appointmentsLoading } = useClientAppointments(c.id)
    ...
    <div className="p-[22px_24px] w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          ...
          {appointmentsLoading ? <div className="h-[120px] bg-surface-2 rounded-[16px] animate-pulse" /> : appointments?.length ? (
            <div className="flex flex-col gap-2">
              {appointments.map(a => (
                <div key={a.id} className="bg-surface-2 border border-line rounded-[14px] px-[13px] py-[11px]">
                  <div className="flex items-center justify-between gap-2">
                    <div className="font-bold text-[13px] text-ink">{a.service_name ?? 'Appointment'}</div>
                    <span className="text-[10px] font-bold uppercase text-muted">{a.status}</span>
                  </div>
                  <div className="text-[11.5px] text-muted mt-1">{new Date(a.starts_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })} · {new Date(a.starts_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</div>
                  <div className="text-[11px] text-muted mt-1">GH₵{a.total_price.toLocaleString()} total · GH₵{a.deposit_paid.toLocaleString()} deposit</div>
                </div>
              ))}
            </div>
          ) : <div className="bg-surface-2 border border-line rounded-[16px] p-[22px] text-center text-[12px] text-muted">No appointments recorded yet.</div>}
        </div>
      </div>
    </div>
  )
